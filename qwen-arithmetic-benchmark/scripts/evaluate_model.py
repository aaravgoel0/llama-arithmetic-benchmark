...
def make_chat_inputs(tokenizer, expr, device_str="cuda"):
    ...
    return tokenizer(text, return_tensors="pt", padding=False).to(device_str)
...
def solve_batch(model, tokenizer, problems, device_str="cuda"):
    ...
    batched_input_ids = torch.stack([pad_left(t, max_len, pad_id) for t in input_ids_list], dim=0)
    batched_attention = torch.stack([pad_left(t, max_len, 0) for t in attn_list], dim=0)
